            ...
            <div>
              ...
              <div className='grid md:grid-cols-3 gap-8 md:gap-2 mt-8'>
                ...
                {topupAmount.map((item,i)=>(
                    <div key={i} id={item.id} onClick={handleClick} className={`bg-cyan-50 border-2 border-transparent p-5 rounded-2xl relative cursor-pointer ${item.id===topUp?"active":''}`}>
                      <h3 onClick={(e)=>e.preventDefault()} className='mb-3 pointer-events-none'>{item.amount} <span className='font-thin'>{item.type}</span></h3>
